...
def write_batch(con: duckdb.DuckDBPyConnection, batch: list[dict]) -> None:
    ...
    Dùng INSERT ... ON CONFLICT DO UPDATE để đảm bảo idempotent:
    nếu cùng event_id được ghi lại (replay sau crash), dữ liệu được
    cập nhật thay vì tạo hàng trùng.
    """
    con.executemany(
        f"""INSERT INTO {TABLE}
            (event_id, ticket_id, customer_id, customer_name, event_type,
             latency_ms, event_time, _ingested_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
